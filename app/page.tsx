            ...
            {VISA_TYPES.map((v) => {
              ...
              return (
                <Link
                  key={v.slug}
                  href={`/visa/${v.slug}`}
                  className="group relative overflow-hidden rounded-2xl border border-gray-100 bg-white hover:border-transparent hover:shadow-xl transition-all duration-300"
                >
                  {/* Colour accent bar — thin on mobile, standard on desktop */}
                  <div className={`h-1 lg:h-2 w-full bg-gradient-to-r ${colors.from} ${colors.to}`} />

                  {/* Layout: horizontal row on mobile/tablet, vertical column on lg+ */}
                  <div className="flex lg:flex-col items-center lg:items-start gap-4 lg:gap-0 p-4 lg:p-6 group-hover:bg-gray-50 transition-colors">
                    {/* Icon box */}
                    ...
                    <div className="flex-1 lg:flex-none min-w-0">
                      <h3 className="text-sm lg:text-lg font-bold text-gray-900 mb-1 lg:mb-2 leading-snug">
                        {v.name}
                      </h3>
                      <p className="text-xs lg:text-sm text-gray-500 leading-relaxed line-clamp-2 lg:line-clamp-3 lg:mb-4">
                        {v.shortDescription}
                      </p>
                      <div className={`mt-1.5 lg:mt-0 inline-flex items-center gap-1 lg:gap-1.5 text-xs font-bold ${colors.text} group-hover:gap-2.5 transition-all`}>
                        Full Guide
