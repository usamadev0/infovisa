    ...
    <>
      ...
      <section id="visa-types" className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Mobile/tablet (< lg): 1-column horizontal cards — icon left, text right */}
          {/* Desktop (lg+): 5-column vertical card grid */}
            ...
            {VISA_TYPES.map((v) => {
              ...
              return (
                <Link
                  key={v.slug}
                  href={`/visa/${v.slug}`}
                  // "block" forces display:block on <a> so children stack correctly
                  className="block group relative overflow-hidden rounded-2xl border border-gray-100 bg-white hover:border-transparent hover:shadow-xl transition-all duration-300"
                >
                  {/* ── Desktop: top accent bar (hidden on mobile) ── */}
                  <div className={`hidden lg:block h-2 w-full bg-gradient-to-r ${colors.from} ${colors.to}`} />

                  {/* ── Mobile: left accent bar (absolutely positioned, hidden on desktop) ── */}
                  <div className={`lg:hidden absolute inset-y-0 left-0 w-1.5 bg-gradient-to-b ${colors.from} ${colors.to}`} />

                  {/* Card body: row on mobile/tablet, column on desktop */}
                  <div className="flex lg:flex-col items-center lg:items-start gap-4 lg:gap-0 pl-6 pr-4 py-3.5 lg:p-6 group-hover:bg-gray-50 transition-colors">
                    {/* Icon */}
                    <div className={`shrink-0 w-12 h-12 lg:w-14 lg:h-14 rounded-xl lg:rounded-2xl flex items-center justify-center lg:mb-4 bg-gradient-to-br ${colors.from} ${colors.to} shadow-md`}>
                      <IconComp className="w-5 h-5 lg:w-7 lg:h-7 text-white" />
                    </div>

                    {/* Text */}
                    <div className="flex-1 lg:flex-none min-w-0">
                      <h3 className="text-sm lg:text-lg font-bold text-gray-900 mb-0.5 lg:mb-2 leading-snug">
                        {v.name}
                      </h3>
                      <p className="text-xs lg:text-sm text-gray-500 leading-relaxed line-clamp-2 lg:line-clamp-3 lg:mb-4">
                        {v.shortDescription}
                      </p>
                      <div className={`mt-1 lg:mt-0 inline-flex items-center gap-1 lg:gap-1.5 text-xs font-bold ${colors.text} group-hover:gap-2.5 transition-all`}>
                        Full Guide
